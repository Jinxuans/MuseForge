...
import { getTaskApiProfile, markInterruptedOpenAIRunningTasks } from './store/tasks/taskDomain'
import { canCancelQueuedServerTask } from './store/tasks/backendTaskExecution'
...
describe('reused task API profile', () => {
  ...
  it('resolves a task API profile by stored profile id', () => {
    const resolved = getTaskApiProfile(settings, task({ apiProvider: 'fal', apiProfileId: falProfile.id }))

    expect(resolved?.id).toBe(falProfile.id)
  })

  it('does not resolve a task API profile by stored name or model', () => {
    const resolved = getTaskApiProfile(settings, task({
      apiProvider: 'fal',
