...
import { cancelQueuedServerTask, editOutputs, getPersistedState, importData, initStore, regenerateAgentAssistantMessage, removeTask, reuseConfig, submitAgentMessage, submitTask, useStore } from './store'
...
describe('reused task API profile', () => {
  ...
  it('asks whether to submit with current API profile when the reused API profile is missing', async () => {
    ...
    expect(state.setConfirmDialog).toHaveBeenCalledWith(expect.objectContaining({
      title: '找不到 API 配置',
      message: '找不到复用任务所使用的 API 配置「未知配置」，要使用当前的 API 配置「TokFlux」提交任务吗？',
