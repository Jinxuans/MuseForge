...
function buildQuery(input: Record<string, string | number | null | undefined>) {
...
function normalizeEnvelope<T>(value: unknown): ApiEnvelope<T> {
  ...
  if (record.ok === true || record.ok === false) return record
...
function assertConfigured(baseUrl: string) {
  if (!baseUrl) throw new Error('尚未配置广场 API 地址，请设置 VITE_SQUARE_API_URL')
}

class HttpSquareApiClient {
  ...
  async createShare(input: SquareCreateShareInput): Promise<SquareCreateShareResult> {
    ...
    for (const asset of input.assets) {
      form.append(`asset:${asset.clientAssetId}:original`, asset.original, createAssetFileName(asset.clientAssetId, 'original', asset.original))
      form.append(`asset:${asset.clientAssetId}:thumb`, asset.thumbnail, createAssetFileName(asset.clientAssetId, 'thumb', asset.thumbnail))
    }
  ...
  private async ensureIdentity(): Promise<SquareIdentity> {
    ...
    const response = await fetch(`${this.baseUrl}/api/v1/identity`, { method: 'POST' })
...
function createAssetFileName(clientAssetId: string, variant: 'original' | 'thumb', blob: Blob) {
...
export const squareApiClient = new HttpSquareApiClient(getSquareApiBaseUrl())

export function isSquareApiConfigured() {
  return squareApiClient.isConfigured()
}

export function resolveSquareAssetUrl(pathOrUrl: string) {
  return squareApiClient.resolveUrl(pathOrUrl)
}

export function summarizeSquareShare(item: SquareShareSummary) {
