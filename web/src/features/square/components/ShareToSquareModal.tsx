...
function splitTagParts(value: string) {
  return value
    .split(/[，,\n;]/)
...
function normalizeTags(values: string[]) {
  return Array.from(new Set(values.map((tag) => tag.trim().slice(0, MAX_TAG_LENGTH)).filter(Boolean))).slice(0, MAX_TAG_COUNT)
}

function appendTags(current: string[], value: string) {
  return normalizeTags([...current, ...splitTagParts(value)])
}

function resolveTaskTitle(prompt: string) {
...
export default function ShareToSquareModal() {
  ...
  const bodyText = isTaskTarget ? task?.prompt.trim() || '该任务没有提示词' : target.content.trim()
    ...
    try {
      ...
      const shareInput = target.kind === 'task'
        ? await (() => {
            ...
            return buildTaskShareInput({ task, tasks, title, tags: submitTags })
          })()
      ...
      await squareApiClient.createShare(shareInput)
  ...
  return (
    <ModalFrame
      onClose={close}
      overlayClassName="fixed inset-0 z-[74] flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm"
      panelClassName="relative z-10 flex max-h-[88vh] w-full max-w-lg flex-col overflow-hidden rounded-3xl border border-white/60 bg-white/95 shadow-2xl ring-1 ring-black/5 dark:border-white/[0.08] dark:bg-gray-900/95 dark:ring-white/10"
    >
        <div className="flex items-center justify-between gap-3 border-b border-gray-100 px-5 py-4 dark:border-white/[0.08]">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {isTaskTarget ? '发布成功图任务、任务链和缩略图' : '发布可被复制和复用的提示词'}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
          ...
          <label className="block">
            ...
            <div className="mt-1.5 flex min-h-10 w-full flex-wrap items-center gap-1.5 rounded-xl border border-gray-200/90 bg-white px-2.5 py-1.5 text-sm text-gray-700 shadow-sm transition focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-500/30 dark:border-white/[0.08] dark:bg-gray-950 dark:text-gray-200">
              {tags.map((tag) => (
                <span key={tag} className="inline-flex h-7 max-w-full items-center gap-1 rounded-full bg-blue-50 px-2.5 text-xs font-medium text-blue-600 ring-1 ring-blue-100 dark:bg-blue-500/10 dark:text-blue-200 dark:ring-blue-400/20">
            ...
            <p className="mt-1.5 text-[11px] leading-4 text-gray-400 dark:text-gray-500">按 Enter 创建标签，支持逗号或换行批量输入</p>
          </label>
          ...
          {isTaskTarget && (
            <div className="rounded-2xl border border-gray-200/80 bg-white/70 px-4 py-3 text-xs leading-5 text-gray-500 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-400">
              将上传生成输出图、缩略图、必要参数和编辑任务链；不会上传 API Key、baseUrl 或错误调试快照。
            </div>
          )}
