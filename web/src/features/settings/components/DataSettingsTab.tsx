...
import { ExportIcon, ImportIcon, TrashIcon } from '../../../shared/ui/icons'
...
interface DataSettingsTabProps {
  ...
  onExportConfigChange: (checked: boolean) => void
  onExportTasksChange: (checked: boolean) => void
  onImportConfigChange: (checked: boolean) => void
  onImportTasksChange: (checked: boolean) => void
  onClearConfigChange: (checked: boolean) => void
  onClearTasksChange: (checked: boolean) => void
  onExport: () => void
  onImportClick: () => void
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-4 dark:border-white/[0.06] dark:bg-white/[0.02] space-y-4 shadow-sm">
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-3">
          <Checkbox checked={exportConfig} onChange={onExportConfigChange} label="包含配置" />
          <Checkbox checked={exportTasks} onChange={onExportTasksChange} label="包含任务和图片" />
        </div>
        <button
          onClick={onExport}
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-4 dark:border-white/[0.06] dark:bg-white/[0.02] space-y-4 shadow-sm">
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-3">
          <Checkbox checked={importConfig} onChange={onImportConfigChange} label="包含配置" />
          <Checkbox checked={importTasks} onChange={onImportTasksChange} label="包含任务和图片" />
        </div>
        <button
      ...
      <div className="rounded-2xl border border-red-100/50 bg-red-50/30 p-4 dark:border-red-500/10 dark:bg-red-500/5 space-y-4 shadow-sm">
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-3">
          <Checkbox checked={clearConfig} onChange={onClearConfigChange} label="包含配置" tone="danger" />
          <Checkbox checked={clearTasks} onChange={onClearTasksChange} label="包含任务和图片" tone="danger" />
        </div>
        <button
