...
import type { CustomProviderForm } from './customProvider'
...
type CustomProviderEditorProps = {
  ...
  onLlmPromptTooltipVisibleChange: (visible: boolean) => void
  onLlmPromptTooltipTouchStart: () => void
  onClearLlmPromptTooltipTimer: () => void
  onPasteJson: () => void
  onFormChange: (patch: Partial<CustomProviderForm>) => void
  onSave: () => void
}

export default function CustomProviderEditor({
  ...
  onClose,
  onCopyLlmPrompt,
  onLlmPromptTooltipVisibleChange,
  onLlmPromptTooltipTouchStart,
  onClearLlmPromptTooltipTimer,
  onPasteJson,
  onFormChange,
  onSave,
  ...
  return createPortal(
    <ModalFrame
      onClose={onClose}
      overlayClassName="fixed inset-0 z-[100] flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm animate-overlay-in"
      panelClassName="relative z-10 w-full max-w-md rounded-3xl border border-white/50 bg-white/95 p-5 shadow-2xl ring-1 ring-black/5 animate-modal-in dark:border-white/[0.08] dark:bg-gray-900/95 dark:ring-white/10 flex flex-col h-[85vh] sm:h-[680px] max-h-[90vh] overflow-hidden"
    >
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="relative inline-flex">
                <button
                  ...
                  onMouseEnter={() => onLlmPromptTooltipVisibleChange(true)}
                  onMouseLeave={() => onLlmPromptTooltipVisibleChange(false)}
                  onFocus={() => onLlmPromptTooltipVisibleChange(true)}
                  onBlur={() => onLlmPromptTooltipVisibleChange(false)}
                  onTouchStart={onLlmPromptTooltipTouchStart}
                  onTouchEnd={onClearLlmPromptTooltipTimer}
                  onTouchCancel={onClearLlmPromptTooltipTimer}
